      ...
      <Card className="relative overflow-hidden p-0">
        ...
        <div className="grid gap-4 p-5 md:grid-cols-[0.82fr_1.18fr]">
          <div className="rounded-[1.25rem] border border-[var(--border)] bg-[var(--surface-muted)] p-4">
            ...
            <div className="mt-8 flex h-36 items-end gap-2">
              {[34, 52, 44, 68, 73, 88, 76, 94].map((height, index) => (
                <div key={index} className="flex-1 rounded-t-full bg-[image:var(--gradient-brand)] opacity-80" style={{ height: `${height}%` }} />
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-4">
            {pipeline.map((item) => (
              <div key={item.label} className="rounded-[1.25rem] border border-[var(--border)] bg-[rgba(16,23,34,0.72)] p-4">
                ...
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--surface-muted)]">
                  <div className="h-full rounded-full bg-[image:var(--gradient-brand)]" style={{ width: item.width }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <Card className="p-5">
        ...
        <div className="mt-6 grid gap-3">
          {recommendations.map((item) => (
            <div key={item} className="flex items-start gap-3 rounded-[1rem] border border-[var(--border)] bg-[var(--surface-muted)] p-3">
